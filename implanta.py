...
def processar_ata(ata_path, log_file):
    try:
        ...
        for participante in ata["integrantes"]:
            ...
            while time.time() < timeout:
                ...
                if nenhum_resultado and "nenhum resultado encontrado" in nenhum_resultado[0].text.lower():
                    log_file.write(f"{ata_path}: Nenhum resultado encontrado para: {participante}\n")
                    encontrado = False
                    break
                ...
                time.sleep(0.5)
            if not encontrado:
                log_file.write(f"{ata_path}: Participante não encontrado na lista: {participante}\n")
                continue
        ...
        if os.path.exists(pdf_path):
            ...
            log_file.write(f"{ata_path}: Arquivo PDF não encontrado para anexar: {pdf_path}\n")
        ...
        try:
            ...
            log_file.write(f"{ata_path}: Registro salvo com sucesso!\n")
            time.sleep(1)
        except Exception as e:
            log_file.write(f"{ata_path}: Não foi possível clicar no botão Salvar para finalizar o processo. {e}\n")
        driver.quit()
    except Exception as e:
        log_file.write(f"{ata_path}: Erro geral ao processar: {e}\n")

# --- MULTI-ARQUIVOS ---
if __name__ == "__main__":
    Tk().withdraw()
    ata_paths = filedialog.askopenfilenames(filetypes=[("Atas TXT", "*.txt")])
    if not ata_paths:
        print("Nenhum arquivo selecionado.")
        exit()

    with open("implanta_execucao.log", "a", encoding="utf-8") as log_file:
        for ata_path in ata_paths:
            log_file.write(f"\n--- Processando: {ata_path} ---\n")
            print(f"Processando: {ata_path}")
            processar_ata(ata_path, log_file)
            log_file.flush()
            print(f"Finalizado: {ata_path}")
